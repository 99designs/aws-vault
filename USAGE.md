...
Note that `credential_process` is designed for retrieving master credentials, while aws-vault outputs STS credentials by default. The AWS CLI/SDK uses the master credentials from the `credential_process` to generate STS credentials itself. So when using this feature it may make more sense to output master credentials by using a profile without a role and the `--no-session` argument, for example:
